...
import { IEditStaffUserInput } from "@/lib/server/staff/interface";
...
async function handler(request: Request) {
  ...
  const validationResponse = staffValidator.editStaff(body, request);
  if (!validationResponse.valid) return validationResponse.response!;

  return await staffController.updateStaff(
    { ...body, staffId: validationResponse.staffId! },
