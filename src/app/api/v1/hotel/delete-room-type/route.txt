...
async function handler(request: Request) {
  ...
  const validationResponse = roomTypeValidator.deleteRoomType(request);
  if (!validationResponse.valid) return validationResponse.response!;

  console.log("validationResponse.roomTypeId", validationResponse.roomTypeId);

  return await roomTypeController.deleteRoomType(
    validationResponse.roomTypeId!,
