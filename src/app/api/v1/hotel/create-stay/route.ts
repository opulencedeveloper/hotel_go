...
async function handler(request: Request) {
  ...
  const body: IAddStayUserInput = await request.json();
  ...
  const validationResponse = stayValidator.addStay(body);
  if (!validationResponse.valid) return validationResponse.response!;

  return await stayController.addStay(body, { hotelId: auth.hotelId });
}
