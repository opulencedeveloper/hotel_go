...
import { IAddRoomTypeUserInput } from "@/lib/server/roomType/interface";
...
async function handler(request: Request) {
  ...
  const body: IAddRoomTypeUserInput = await request.json();
  ...
  const validationResponse = roomTypeValidator.addRoomType(body);
  if (!validationResponse.valid) return validationResponse.response!;

  return await roomTypeController.addRoomType(body, { hotelId: auth.hotelId });
}
