...
import { IEditFacilityUserInput } from "@/lib/server/facility/interface";
...
async function handler(request: Request) {
  ...
  const body: IEditFacilityUserInput = await request.json();
  ...
  const validationResponse = facilityValidator.editFacility(body, request);
  if (!validationResponse.valid) return validationResponse.response!;

  return await facilityController.editFacility(
    { ...body, facilityId: new Types.ObjectId(validationResponse.facilityId!) },
