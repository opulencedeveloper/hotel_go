...
export default function RevenueBreakdown({
  ...
  const scheduledServicesPercentage = totalRevenue > 0 ? ((scheduledServicesRevenue / totalRevenue) * 100).toFixed(0) : '0';
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <span className="font-medium text-secondary-900">Scheduled Services</span>
            </div>
            <div className="text-right">
              <p className="font-semibold text-secondary-900">{formatPrice(scheduledServicesRevenue, currency)}</p>
              <p className="text-sm text-secondary-500">{scheduledServicesPercentage}%</p>
            </div>
          </div>
          <div className="w-full bg-secondary-200 rounded-full h-2">
            <div className="bg-indigo-500 h-2 rounded-full" style={{ width: `${scheduledServicesPercentage}%` }}></div>
          </div>
