...
export default function StaysTabs({ selectedTab, onTabChange, stays }: StaysTabsProps) {
  ...
  return (
    <div className="border-b border-gray-200 overflow-x-auto -mx-4 sm:mx-0 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
      <nav className="-mb-px flex space-x-4 sm:space-x-8 px-4 sm:px-0 min-w-max sm:min-w-0 scroll-smooth">
        {tabs.map((tab) => (
          <button
            key={tab.value}
            ...
            } whitespace-nowrap py-2 px-1 sm:px-1 border-b-2 font-medium text-sm flex items-center flex-shrink-0`}
          >
            {tab.icon}
